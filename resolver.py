...
def pitDetermination(board):
    ...
    for i in range(6):
        neededValues.insert(i, (finalStore(board, i)))  # 1. Putting the final seed in the player store
        #if blockOpp(board, i):    # 2. Block the opponent from putting a seed in there store
        ...
        #  elif canCollect(board, i) == True:  # 4. Move the right most non zero pit
            #  print("Move the seeds in pit", 6-i, "to empty your pit")

    finalGemInStore = {
        "pit1A": [1, 0, neededValues[0]],
        "pit2A": [1, 1, neededValues[1]],
        "pit3A": [1, 2, neededValues[2]],
        "pit4A": [1, 3, neededValues[3]],
        "pit5A": [1, 4, neededValues[4]],
        "pit6A": [1, 5, neededValues[5]],
    }

    return finalGemInStore
